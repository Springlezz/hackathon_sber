...
export async function post(db, { userId, body }) {
    ...
    const [{ creator }] = await dbGet(db, 'SELECT creator FROM events WHERE id = ?', body.id);
    if (creator === userId) return [400, { error: 'Вы не можете присоединиться к событию, которое создали.' }];

    const [event] = await dbGet(db, 'SELECT id FROM event_users WHERE user_id = ? AND event_id = ?', userId, body.id);
    if (event) return [400, { error: 'Вы уже присоединились к этому событию.' }];
